...
### Phase 6 — プリセットの初期選択と、削除済み報酬の掃除

- [x] P6-1 起動時にプリセットを自動選択（`default` / `デフォルト` を優先、無ければ最古の作成日時）
- [x] P6-2 アプリから報酬を削除したとき、プリセットからも項目を除去
- [x] P6-3 Web / 他アプリで削除された報酬は「（削除済み）」を一度だけ表示し、その場でプリセットから除去（次回は表示されない）
- [x] P6-4 「状態」列の見出しを外し、（削除済み）の注記だけ出す形に
- [x] P6-5 一覧の取得に失敗しているときは掃除も「削除済み」表示も行わない（プリセットを壊さないための安全弁）
- [x] P6-6 ビルド確認
- [ ] P6-7 実機動作確認
...
| 15 | ~~報酬を削除してもプリセットの項目は残る~~ → P6 で変更。アプリから削除した場合は即座にプリセットからも除去し、Web / 他アプリで削除された場合は「（削除済み）」を一度表示してから除去する | **P6 で変更** |
| 16 | `Setting` / `AppLog` タブも `StackPanel` で包まれたまま。今回スクロール不具合が出たのは CP タブだけだが、同じ構造なので将来内容が増えると同様の問題が起きうる | 本改修の対象外 |
| 17 | プリセット項目の掃除は「選択中プリセットの内訳を組み立てた直後」に全プリセットを対象に実行する。表示中の内容には（削除済み）が残り、次に開いたときには消えている、という挙動になる | 仕様として確定 |
| 18 | 掃除によってプリセットの件数表示（ComboBox の「〇件」）が一時的に古いままになる。次に `ReloadPreset` が走る操作（保存・適用・削除）で解消する | 軽微・未対応 |
